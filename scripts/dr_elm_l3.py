...
delta_l3 is re-multiplied by sd_y (physical units). mu_y/sd_y frozen on the
...
# delta grid in center-scaled (z) space: fractions of sd_y. The target is solved
# on z (r ~ O(1)); the old absolute-watt grid {10,100,500} would make the weights
# sqrt(r^2+delta^2) ~ delta (flat) and degenerate L3 into Ridge. {0.05,0.1,0.5}*
# sd_y keeps delta < sd_y, preserving the L3 emphasis on large residuals.
...
def train_elm_l3(
    ...
    def fit_score(X_fit, y_fit, X_val, y_val, IW, bias, combo):
        ...
        beta = l3_solve(elm_sigmoid(X_fit @ IW.T + bias), (y_fit - mu) / sd, lam, dlt)
    ...
    def refit(X_full, y_full, IW, bias, combo):
        ...
        return l3_solve(elm_sigmoid(X_full @ IW.T + bias), (y_full - mu) / sd, lam, dlt), None
...
def train_elm_l3_grid(
    ...
    sel_dict = {
        ...
        "lambda_l3": best_lam, "delta_l3": best_delta * sd_y,
    }
...
def main() -> None:
    run_elm(
        ...
        grid_print=f"L3 2-pass: lam_grid={LAMBDA_GRID}, delta_grid(z)={DELTA_GRID_Z}",
    )
